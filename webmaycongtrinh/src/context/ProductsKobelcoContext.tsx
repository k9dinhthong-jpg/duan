...
const productsKobelcoApiPath = getConfiguredApiPath(
  "VITE_API_PRODUCTS_KOBELCO_PATH",
  "/api/products/kobelco",
);
...
export function ProductsKobelcoProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function fetchProductsKobelco() {
      try {
        const payload = await requestJson(productsKobelcoApiPath);
        ...
        setError(
          unknownError instanceof Error
            ...
            : "Khong the tai du lieu Kobelco tu backend.",
        );
      } finally {
        if (isMounted) {
          setIsLoading(false);
